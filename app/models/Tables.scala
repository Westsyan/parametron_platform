...
trait Tables {
  ...
  lazy val schema: profile.SchemaDescription = Project.schema ++ Sample.schema ++ Species.schema ++ Task.schema ++ User.schema
  ...
  final case class ProjectRow(id: Int, accountid: Int, projectname: String, description: String, createdata: DateTime, samcount: Int, speciesid: Int, species: String)
  ...
  implicit def GetResultProjectRow(implicit e0: GR[Int], e1: GR[String], e2: GR[DateTime]): GR[ProjectRow] = GR{
    prs => import prs._
    ProjectRow.tupled((<<[Int], <<[Int], <<[String], <<[String], <<[DateTime], <<[Int], <<[Int], <<[String]))
  }
  /** Table description of table project. Objects of this class serve as prototypes for rows in queries. */
  class Project(_tableTag: Tag) extends profile.api.Table[ProjectRow](_tableTag, Some("parametron_platform"), "project") {
    def * = (id, accountid, projectname, description, createdata, samcount, speciesid, species) <> (ProjectRow.tupled, ProjectRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(accountid), Rep.Some(projectname), Rep.Some(description), Rep.Some(createdata), Rep.Some(samcount), Rep.Some(speciesid), Rep.Some(species)).shaped.<>({r=>import r._; _1.map(_=> ProjectRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get, _8.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
  ...
  class Sample(_tableTag: Tag) extends profile.api.Table[SampleRow](_tableTag, Some("parametron_platform"), "sample") {
   ...
   *  @param id Database column id SqlType(INT), AutoInc
   *  @param speciesname Database column speciesname SqlType(VARCHAR), Length(255,true)
   *  @param accountid Database column accountid SqlType(INT)
   *  @param createdata Database column createdata SqlType(DATETIME)
   *  @param state Database column state SqlType(INT) */
  final case class SpeciesRow(id: Int, speciesname: String, accountid: Int, createdata: DateTime, state: Int)
  /** GetResult implicit for fetching SpeciesRow objects using plain SQL queries */
  implicit def GetResultSpeciesRow(implicit e0: GR[Int], e1: GR[String], e2: GR[DateTime]): GR[SpeciesRow] = GR{
    prs => import prs._
    SpeciesRow.tupled((<<[Int], <<[String], <<[Int], <<[DateTime], <<[Int]))
  }
  /** Table description of table species. Objects of this class serve as prototypes for rows in queries. */
  class Species(_tableTag: Tag) extends profile.api.Table[SpeciesRow](_tableTag, Some("parametron_platform"), "species") {
    def * = (id, speciesname, accountid, createdata, state) <> (SpeciesRow.tupled, SpeciesRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(speciesname), Rep.Some(accountid), Rep.Some(createdata), Rep.Some(state)).shaped.<>({r=>import r._; _1.map(_=> SpeciesRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))

    /** Database column id SqlType(INT), AutoInc */
    val id: Rep[Int] = column[Int]("id", O.AutoInc)
    /** Database column speciesname SqlType(VARCHAR), Length(255,true) */
    val speciesname: Rep[String] = column[String]("speciesname", O.Length(255,varying=true))
    /** Database column accountid SqlType(INT) */
    val accountid: Rep[Int] = column[Int]("accountid")
    /** Database column createdata SqlType(DATETIME) */
    val createdata: Rep[DateTime] = column[DateTime]("createdata")
    ...
    /** Primary key of Species (database name species_PK) */
    val pk = primaryKey("species_PK", (id, accountid))
  }
   ...
   *  @param createdata Database column createdata SqlType(DATETIME)
   *  @param state Database column state SqlType(INT) */
  final case class TaskRow(id: Int, taskname: String, accountid: Int, projectid: Int, createdata: DateTime, state: Int)
  ...
  implicit def GetResultTaskRow(implicit e0: GR[Int], e1: GR[String], e2: GR[DateTime]): GR[TaskRow] = GR{
    prs => import prs._
    TaskRow.tupled((<<[Int], <<[String], <<[Int], <<[Int], <<[DateTime], <<[Int]))
  }
  /** Table description of table task. Objects of this class serve as prototypes for rows in queries. */
  class Task(_tableTag: Tag) extends profile.api.Table[TaskRow](_tableTag, Some("parametron_platform"), "task") {
    def * = (id, taskname, accountid, projectid, createdata, state) <> (TaskRow.tupled, TaskRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(taskname), Rep.Some(accountid), Rep.Some(projectid), Rep.Some(createdata), Rep.Some(state)).shaped.<>({r=>import r._; _1.map(_=> TaskRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
  ...
  class User(_tableTag: Tag) extends profile.api.Table[UserRow](_tableTag, Some("parametron_platform"), "user") {
